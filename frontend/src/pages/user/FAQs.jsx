...
import {
  ...
  CreditCard, ShieldCheck, Smartphone, ChevronDown, Rocket,
  MessageSquare, Mail, Phone, CheckCircle2,
  ThumbsUp, ThumbsDown, ArrowRight, X,
} from "lucide-react";
import "./FAQs.css";
...
const STOP_WORDS = new Set([
  ...
  "has","have","had","do","does","did","will","can","could","would",
  "should","may","might","shall","i","my","me","we","our","you",
  "your","he","she","they","their","this","that","these","those",
  "not","no","so","if","then","than","when","how","what","which",
  "who","where","why","there","here","about","into","just","also",
  "more","some","any","all","each","other","get","use","need","want",
]);
...
function stem(word) {
  ...
  const step2 = [
    ...
    ["izer","ize"],["alism","al"],["ation","ate"],["ator","ate"],
  ...
  const step3 = [["icate","ic"],["ative",""],["alize","al"],["iciti","ic"],["ical","ic"],["ness",""],["ful",""]];
...
function proximityBonus(tokens, queryTerms) {
  ...
  for (let a = 0; a < queryTerms.length - 1; a++) {
    for (let b = a + 1; b < queryTerms.length; b++) {
      ...
      for (const pa of posA) for (const pb of posB) minDist = Math.min(minDist, Math.abs(pa - pb));
...
const WEIGHT = { question: 3.0, questionStart: 2.0, answer: 1.0, bigram: 1.8, trigram: 2.5, phonetic: 0.6, proximity: 1.0 };
...
function detectIntentBoost(rawQuery, faqCategory) {
  ...
  for (const signal of INTENT_SIGNALS) {
    ...
    for (const re of signal.patterns) {
      if (re.test(rawQuery)) { boost += signal.boost; break; }
    }
  }
  return boost;
}
...
function toId(label) { return safeStr(label).trim().toLowerCase(); }
...
function buildGmailComposeUrl({ to, subject, body }) {
  ...
  params.set("view", "cm"); params.set("fs", "1"); params.set("to", safeStr(to));
  ...
  return (
    ...
    >
      <div className="fq-item__head">
        ...
        <div className="fq-item__ico"><Icon size={13} /></div>
      ...
      <div className="fq-item__body">
        <div className="fq-item__body-inner">
          ...
          <div className="fq-item__foot" onClick={e => e.stopPropagation()}>
            ...
            <button className={`fq-vote fq-vote--yes ${helpful === "yes" ? "fq-vote--active" : ""}`} onClick={e => handleHelpful(e, "yes")}>
              <ThumbsUp size={11} /><span>Yes</span>
            </button>
            <button className={`fq-vote fq-vote--no ${helpful === "no" ? "fq-vote--active" : ""}`} onClick={e => handleHelpful(e, "no")}>
              <ThumbsDown size={11} /><span>No</span>
            </button>
            {helpful === "no" && (
              <a href={gmailUrl} target="_blank" rel="noopener noreferrer" className="fq-vote-cta" onClick={e => e.stopPropagation()}>
...
export default function FAQsPage() {
  ...
  useEffect(() => {
    let alive = true;
    fetchPublicSettings().then(d => { if (alive) setSettings(d); }).catch(() => {});
    return () => { alive = false; };
  }, []);

  useEffect(() => { const t = setTimeout(() => setHeroIn(true), 60); return () => clearTimeout(t); }, []);
  useEffect(() => { setActiveFaq(null); }, [activeCategory, searchQuery]);

  const supportEmail = useMemo(() => safeStr(settings?.support_email) || safeStr(settings?.contact_email) || "support@exersearch.com", [settings]);
  const contactPhone = useMemo(() => safeStr(settings?.contact_phone) || "+63 917 123 4567", [settings]);
          ...
          <div className="fq-wrap">
            ...
            <div className="fq-panel__bar">
              <span className="fq-panel__label">
                {loading ? "Loading…" :
                 loadErr ? <span className="fq-panel__label--err">{loadErr}</span> :
                 searchQuery ? (
                   <><CheckCircle2 size={12} className="fq-panel__check" />
                     <strong>{filteredFaqs.length}</strong>&nbsp;result{filteredFaqs.length !== 1 ? "s" : ""}&nbsp;for&nbsp;<em>"{searchQuery}"</em></>
                 ) : (
              ...
              {(searchQuery || activeCategory !== "all") && !loading && (
                <button className="fq-panel__reset" onClick={() => { setSearchQuery(""); setActiveCategory("all"); }}>
                  <X size={10} /> Reset
                </button>
              )}
            </div>
            ...
            {loading ? (
              ...
              <div className="fq-empty">
                ...
                <p>{loadErr}</p>
                <button className="fq-empty__btn" onClick={() => window.location.reload()}>Retry</button>
              </div>
              ...
              <div className="fq-empty">
                ...
                <button className="fq-empty__btn" onClick={() => { setSearchQuery(""); setActiveCategory("all"); }}>
                  View all articles
                </button>
              </div>
            ) : (
